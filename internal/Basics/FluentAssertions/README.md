...
**FluentAssertions** is a popular .NET assertion library that provides a readable, chainable syntax with rich failure messages. Instead of xUnit's `Assert.Equal(expected, actual)`, you write `actual.Should().Be(expected)` -- making tests read like specifications.
...
| :--- | :--- |
| `actual.Should().Be(expected)` | Exact equality |
| `actual.Should().NotBe(unexpected)` | Inequality |
| `x.Should().BeNull()` | Null reference |
| `x.Should().NotBeNull()` | Non-null reference |
| `cond.Should().BeTrue()` | Boolean true |
| `cond.Should().BeFalse()` | Boolean false |
| `act.Should().NotThrow()` | No exception thrown |
| `act.Should().Throw<T>()` | Specific exception thrown |
| `collection.Should().Contain(elem)` | Collection contains element |
| `collection.Should().HaveCount(n)` | Collection has expected length |

FluentAssertions throws immediately on failure, so every assertion acts as a hard stop -- no need for separate "require" vs "assert" distinction.
...
```csharp
// Compares all public properties recursively — deep structural equality
...
```bash
dotnet test tests/Basics.Tests --filter "FullyQualifiedName~FluentAssertions"
```
